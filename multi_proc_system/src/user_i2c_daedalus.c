...
volatile uint8_t slave_fifo_buffer_number;
...
void I2C_IRQHandler(void){
	...
	static uint8_t fifo_element;
	static uint8_t status_element_flag = 1;
	...
	if(irq_status & I2C_INTSTAT_SLVDESEL){
		...
		case I2C_STAT_SLVCODE_RX:		// Data byte received
			slave_fifo_buffer_number = LPC_I2C->SLVDAT & I2C_SLVDAT_DATAMASK;
			fifo_status = fifo_peek(slave_fifo_buffer_number);
			status_element_flag = 1;
//			if(fifo_status){
//				user_gpio_clear();
//			}
...
//			}
		...
		case I2C_STAT_SLVCODE_TX:		// Data byte sending
			if(status_element_flag){
				status_element_flag = 0;
				LPC_I2C->SLVDAT = fifo_status; //1 - fifo empty, 0 - fifo has elements
			}
			else{
				status_element_flag = 1;
				fifo_pop(slave_fifo_buffer_number, &fifo_element);
				LPC_I2C->SLVDAT = fifo_element;
			}
